...
describe('Noteful API - Notes', function () {
  ...
  describe('GET /api/notes/:id', function () {
    ...
    it('should respond with status 400 and an error message when `id` is not valid', function () {
      return chai.request(app).get('/api/notes/NOT-A-VALID-ID').set('Authorization', `Bearer ${token}`)
        .then(res => {
    ...
    it('should respond with a 404 for an id that does not exist', function () {
      ...
      return chai.request(app).get('/api/notes/DOESNOTEXIST').set('Authorization', `Bearer ${token}`)
        .then(res => {
  ...
  describe('POST /api/notes', function () {
    ...
    it('should create and return a new item when provided valid title and content', function () {
      ...
      let res;
      return chai.request(app).post('/api/notes').send(newItem).set('Authorization', `Bearer ${token}`)
        .then(function (_res) {
          res = _res;
    ...
    it('should return an error when "title" is empty string', function () {
      const newItem = {title: '', userId: user.id};
      return chai.request(app).post('/api/notes').send(newItem).set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res).to.be.json;
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should update the note when provided a valid title', function () {
      ...
      return Note.findOne({userId: user.id})
        .then(_data => {
          data = _data;
          return chai.request(app).put(`/api/notes/${data.id}`).send(updateItem).set('Authorization', `Bearer ${token}`);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
          expect(res).to.be.json;
          ...
          // folderId is not required, so if there is no folderId, we cannot call toString() on undefined
    ...
    it('should respond with status 400 and an error message when `id` is not valid', function () {
      ...
      return chai.request(app).put('/api/notes/NOT-A-VALID-ID').send(updateItem).set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res.body.message).to.equal('The `id` is not valid');
        });
    ...
    it('should respond with a 404 for an id that does not exist', function () {
      ...
      return chai.request(app).put('/api/notes/DOESNOTEXIST').send(updateItem).set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(404);
        });
    });
  ...
  describe('DELETE /api/notes/:id', function () {
    ...
    it('should respond with a 400 for an invalid id', function () {
      return chai.request(app).delete('/api/notes/NOT-A-VALID-ID').set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res.body.message).to.equal('The `id` is not valid');
        });
